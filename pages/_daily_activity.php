        ...
        if($_GET['status'] == "saved"){ ?>
            <script>
                setTimeout(function() { 
                    ...
                    function(isConfirm){
                        if (isConfirm) {
                            window.location.href = "<?= BASEURL; ?>/daily-activity";
                        }
                }); }, 500);
            </script>
  <?php }
...
<div class="container-nd">
        ...
        <form method="POST" action="<?= BASEURL; ?>/daily-activity" class="form-table">
            ...
            <div class="divJudul">
                ...
                <div>
                    <a href="<?= BASEURL; ?>/new-activity" class="linkTransferPg"><i class="fa fa-plus-circle" aria-hidden="true"></i></a>
                    <a href="<?= BASEURL; ?>/daily-activity" class="linkTransferPg"><i class="fa fa-refresh" aria-hidden="true"></i></a>
                </div>
            </div>
                        ...
                        if($cekDailyActivity > 0){
                            ...
                            foreach($dataDailyActivity as $row){ ?>
                                <tr>
                                        ...
                                        <?php
                                            if($row['_status'] == "Pending"){ 
                                                if($getData->cekFollowUp($row['_id_aktifitas'], $row['_id_pekerja']) < 1){ ?>
                                                    <a href="<?= BASEURL; ?>/follow-up-activity/<?= $row['_id_aktifitas']; ?>" class="linkDetail"><i class="fa fa-cogs" aria-hidden="true"></i></a>
                                          <?php }
                                                else {
                                    ...
                                    <td style="text-align:center;"><a href="<?= BASEURL; ?>/detail-activity/<?= $row['_id_aktifitas']; ?>" class="linkDetail"><i class="fa fa-info-circle" aria-hidden="true"></i></a></td>
